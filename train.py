...
from data.train_data import load_data
...
def main():
    ...
    model, preprocess_train, preprocess_val, preprocess_aug = build_model(args)
    ...
    loss = build_loss(args)(args, args.joint_projection_dim).to(args.device)
    ...
    data = load_data(args, (preprocess_train, preprocess_val, preprocess_aug), index_mapping)
    ...
    if is_master(args):
        ...
        num_all_params = round(sum(p.numel() for p in model_without_ddp.model.parameters())/1e6, 2)
        num_visual_params = round(sum(p.numel() for p in model_without_ddp.model.visual.parameters())/1e6, 2)
        num_text_params = round(num_all_params - num_visual_params, 2)
        logging.info('Model structure\n' + str(model))
        logging.info(f'Total Model Parameters (M):\t        {num_all_params}')
        logging.info(f'Image Backbone Parameters (M):\t     {num_visual_params}')
        logging.info(f'Text Backbone Parameters (M):\t      {num_text_params}')
        logging.info(f'Trainable Parameters (M):\t{round(n_trainable_params/1e6, 2)}')
        logging.info(f'Frozen Parameters (M):\t{round(n_frozen_params/1e6, 2)}')
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # 
