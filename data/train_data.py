...
class COCOCaptionsDataset(Dataset):
    def __init__(self, dataset_name, transforms, args):
        ...
        if dataset_name=='mscoco_captions_2014':
            coco_train_root = os.path.join(args.datasets_dir, 'coco/train2014')
            coco_train_json = os.path.join(args.datasets_dir, 'coco/annotations/captions_train2014.json')
            ...
            coco_train_root = os.path.join(args.datasets_dir, 'coco2017/images/train2017')
...
class VisualGenomeRegionsDataset(Dataset):
    def __init__(self, dataset_name, transforms, args):
        self.dataset_name = dataset_name
        self.annotation_file = "regions.json"
        self.annotation_path = os.path.join(args.datasets_dir, "VisualGenome/dataset", self.annotation_file)
        self.image_dir = os.path.join(args.datasets_dir, 'VisualGenome/dataset/images')
    ...
    def __getitem__(self, index):
        ...
        image_name = str(data['image_id']) + ".jpg"
        image_path = os.path.join(self.image_dir, image_name)
        image = Image.open(image_path).convert('RGB')
        # Get the bounding box that contains the relation.
        # This is to remove the irrelevant details in the scene.
...
class OpenImagesV6(Dataset):
    ...
    transforms: callable = None
        Transforms.
    """
    # def __init__(self, image_dir: str, annotation_file: str, transforms: callable = None):
    #     """Initialize dataset."""
    def __init__(self, dataset_name, transforms, args):
        self.dataset_name = dataset_name
        self.annotation_file = os.path.join(args.datasets_dir, "OpenImagesV6/annotations/open_images_train_v6_captions.jsonl")
        self.image_dir = os.path.join(args.datasets_dir, 'OpenImagesV6/images')
        self.transforms = transforms
    ...
    def __getitem__(self, index):
        """Return one sample."""
        data = self.data[index]
        image_name = data["image_id"] + ".jpg"
        image_path = os.path.join(self.image_dir, image_name)
        image = Image.open(image_path)
        caption = data["caption"]
        caption = np.random.choice(caption.split(". "))
    ...
    if args.train_data is not None:
        ...
        for dataset_name in args.train_data.split(','):
            if 'mscoco_captions' in dataset_name:
                ...
                # dataset = VisualGenomeRegionsDataset(dataset_name=dataset_name, transforms=preprocess_fn, args=args)
                # dataset = OpenImagesV6(dataset_name=dataset_name, transforms=preprocess_fn, args=args)
