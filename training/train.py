...
from .distributed import is_master, gather_features, get_gathered_item, gather_features_proj
    ...
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            with forward_context():
                ...
                image_features, text_features, image_features_proj, text_features_proj, logit_scale = model(
                    images, texts, text_only=(args.cache_teacher is not None) or args.w_align==0
                )
                ...
                if args.distributed and args.loss in NEED_GATHER:
                    all_image_features, all_text_features, all_image_features_proj, all_text_features_proj = gather_features_proj(
                        image_features, text_features, image_features_proj, text_features_proj,
                ...
                if args.teacher=='text':
                    ...
                    teacher_features, student_features, teacher_features_proj, student_features_proj = (
                        all_text_features, all_image_features, all_text_features_proj, all_image_features_proj
                    )
                elif args.teacher=='image':
                    # teacher_features, student_features = all_image_features, all_text_features
                    teacher_features, student_features, teacher_features_proj, student_features_proj = (
                        all_image_features, all_text_features, all_image_features_proj, all_text_features_proj
                    )
                ...
                if args.loss in NEED_LOGIT_SCALE:
                    if args.loss in ['UniCL', 'CrossEntropy']:
                        ...
                        align_loss = loss(
                            ...
                            teacher_features_proj, student_features_proj, logit_scale=logit_scale)
                else:
        ...
        if args.cache_teacher is not None:
            ...
            if is_master(args) and (i % 10 == 0 or batch_count == num_batches_per_epoch):
                ...
                log_data = {
                    ...
                    "logit_scale": model.module.model.logit_scale.item() if args.distributed else model.model.logit_scale.item()
                }
                profiling = {
