...
def retrieval_evaluation(model, epoch, preprocess, args, recall_k_list=[1,5,10]):
    ...
    if args.retrieval_data=='mscoco_captions_2014':
        ...
        coco_val_root = os.path.join(args.datasets_dir, 'coco/val2014')
        coco_val_json = os.path.join(args.datasets_dir, 'coco/annotations/captions_val2014.json')
        ...
        from torchvision.datasets.coco import CocoCaptions
        coco_val_root = os.path.join(args.datasets_dir, 'coco2017/images/val2017')
    ...
    for batch_images, batch_texts, inds in tqdm(dataloader, total=n_batches): 
        ...
        with torch.no_grad():
            if args.distributed and not args.horovod:
                batch_image_features = model.module.encode_image(batch_images, projection=True)[-1]
                batch_text_features = model.module.encode_text(batch_texts, projection=True)[-1]
            else:
                batch_image_features = model.encode_image(batch_images, projection=True)[-1]
                batch_text_features = model.encode_text(batch_texts, projection=True)[-1]
